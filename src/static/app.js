document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Reset activity options (preserve placeholder in HTML)
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants HTML (each participant gets a small remove button)
        const participantsHtml = details.participants && details.participants.length
          ? `<ul class="participants-list">${details.participants.map(p => `<li class="participant-item"><span class="participant-email">${p}</span><button class="participant-remove" data-activity="${name}" data-email="${p}" aria-label="Remove ${p}">✖</button></li>`).join("")}</ul>`
        ...
        removeButtons.forEach(btn => {
          btn.addEventListener('click', async (event) => {
            ...
            if (!confirm(`Remove ${emailToRemove} from ${activityName}?`)) return;
